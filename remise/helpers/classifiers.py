...
class BayesClassifier:
    ...
    def predict(self, testdata1array, images_names, expected_labels1array=None, gen_output=False):
        """
        ...
        class_risks = np.zeros_like(classProbDensities)
        for i in range(self.n_classes):
            for j in range(self.n_classes):
                class_risks[:, i] += classProbDensities[:, j] * self.costs[i][j]

        # on veut minimiser l'erreur
        predictions = np.argmin(class_risks, axis=1).reshape(testDataNSamples, 1)

        if np.asarray(expected_labels1array).any():
            errors_indexes = an.calc_erreur_classification(expected_labels1array, predictions, images_names, gen_output)
        else:
...
class BayesClassify_APP2:
    def __init__(self, image_names, data2train, data2test=None, ndonnees_random=5000,
        ...
        self.predictRandom, _ = self.classifier.predict(self.donneesTestRandom, image_names)  # classifie les données de test1
        if np.asarray(data2test).any():   # classifie les données de test2 si présentes
            self.predictTest, self.error_indexes = \
                self.classifier.predict(data2test.data1array, image_names, data2test.labels1array, gen_output=gen_output)
        else:
            self.predictTest = []
            self.error_indexes = []
        if view:
...
class PPVClassifier:
    def __init__(self, data2train, n_neighbors=1, metric='minkowski',
        ...
        self.kNN = KNN(n_neighbors, metric=metric, p=2)  # minkowski correspond à distance euclidienne lorsque le paramètre p = 2
...
class PPVClassify_APP2:
    def __init__(self, image_names, data2train, data2test=None, n_neighbors=1, metric='minkowski', ndonnees_random=5000,
        ...
        self.classifier = PPVClassifier(data2train, n_neighbors=n_neighbors, metric=metric,
                                        ...
                                        view=True)
        self.donneesTestRandom = an.genDonneesTest(ndonnees_random, data2train.extent)
        self.predictRandom, _ = self.classifier.predict(self.donneesTestRandom, image_names)  # classifie les données de test
        if np.asarray(data2test).any():   # classifie les données de test2 si présentes
            self.predictTest, self.error_indexes = \
                self.classifier.predict(data2test.data1array, image_names, data2test.labels1array, gen_output=gen_output)
        else:
...
class NNClassifier:
    ...
    def predict(self, testdata1array, image_names, expected_labels1array=None, savename='', gen_output=False):
        ...
        if np.asarray(expected_labels1array).any():
            errors_indexes = an.calc_erreur_classification(expected_labels1array, predictions, image_names, gen_output)
        else:
...
class NNClassify_APP2:
    def __init__(self, image_names, data2train, data2test, n_layers, n_neurons, innerActivation='tanh', outputActivation='softmax',
        ...
        self.donneesTestRandom = an.genDonneesTest(ndonnees_random, data2train.extent)
        self.predictRandom, _ = self.classifier.predict(testdata1array=self.donneesTestRandom, image_names=image_names)  # classifie les données de test1
